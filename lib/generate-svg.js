...
// Helpers
const { VAR_NAMESPACE, VAR_NAMESPACE_VALUE, hasVariables, hasVarNamespace, addVarNamespace } = require('./helpers/variables');
const { filterTransferableAttributes, addXMLNSAttributesToElement } = require('./helpers/attributes');
...
module.exports = (files = [], options = {}) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        Promise.all(files.map((file) => new Promise((resolve) => {
            const id = `${options.sprite.prefix}${idify(path.basename(file, path.extname(file)))}`;
            ...
            items.forEach((item) => {
                ...
                // Attributes
                const attributes = filterTransferableAttributes(sprite.attributes);
                addXMLNSAttributesToElement(attributes, svg);
                ...
                if ( options.sprite.generate.symbol ) {
                    ...
                    // Add the XMLNS attributes to the symbol as well
                    // as they will need to passed to the styles generator as well
                    addXMLNSAttributesToElement(attributes, symbol);
